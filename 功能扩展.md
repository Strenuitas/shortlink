...
# 消息队列重构短链接监控工呢 ！！！！😧😧😧😧😧😧😧😧😧😧😧😧😧😧😧😧😧😧😧😧😧😧😧😧😧😧😧重要
初始的是跳转之后同步记录统计信息，这样的话如果海量访问短链接直接访问数据库，会把数据库打穿，因此需要引入消息队列削峰，轻量级的应用，能不引组件就不引，所以选用Redis充当消息队列,跳转和记录监控异步，前者返回请求，后者有一个异步线程一直在获取消息执行记录监控逻辑。
...
```
3:然后是代码里面写对应的流程  
    1：首先是创建好RedisStream的config配置类  
        引入两个配置 一个RedisConnectionFactory和Redis消费者，然后是topic和group，对应yaml文件里写好的stream的topic和group  
        创建线程池，因为消息消费者执行逻辑是通过线程池执行，如果不自定义会使用默认的，因此我们自定义一个。  
        然后是监听的绑定，设置监听时候的配置：一次去消息队列里面拉取多少条消息，拉到内存里面，由哪个执行者执行，这里使用的是我们自定义的线程池，如果stream里面没有消息了，那我们不能一直去拉取消息了，让他pullTimeout类似sleep一段时间，不能大于Redis的全局Timeout时间，否则会超时  
``` java
        StreamMessageListenerContainer.StreamMessageListenerContainerOptions<String, MapRecord<String, String, String>> options =
                StreamMessageListenerContainer.StreamMessageListenerContainerOptions
                        ...
                        .build();
```  
接下来创建对应的绑定监听：通过连接工厂以及上面我们写好的配置绑定对应的监听也就是我们写好的消费者：  
``` java
...
```  
然后是监控的业务，相当于把统计的具体业务全部封装到消息里面去了，然后通过我们创建好的生产者类执行发送流程，
``` java
...
```
发送完之后然后消费者就能监听到了，因此之前统计业务全部转到消费者组里面去了,消息消费完后，也就是获取到MySQL执行完发送的ACK后，就把消息给删掉，因为stream不足之处在于当消息队列越来越大的时候，RocketMQ消息内容是存在磁盘里面，空间大无所谓，存储三天就会自动删除，但是Stream则是存储在redis，存储空间宝贵  
我们不能容忍消费过的消息一直存放在redis里面。
# 使用消息队列会有数据延迟和幂等的问题 前者是消息消费写的过程会有一定的延迟，即用户跳转完后，统计消息不是同步写入，而是有一定延迟。
使用了消息队列保证了高可用
# 任务 可以选择使用mq或者redis的stream实现消息队列 即 前者：rocketMQ，kafka

# 解决幂等逻辑。
场景：当消息队列重复出现消费问题情况下，应该如何保障数据准确性。   比如说一个用户访问了一次，但是记录了多条。 
问题原因：网络问题：
          生产重试：
如何解决 幂等
写一个handler，然后里面去判断消息是否被消费过，这里使用一个setIfAbsent：stringRedisTemplate.opsForValue().setIfAbsent(key, "0", 2, TimeUnit.MINUTES)，如果幂等标识存在则返回已消费过，直接return如果幂等标识不存在则新建。这样的话如果消费的时候，就会先去判断Redis里面是否已经有幂等标识了，如果有了就    不在消费。但是这样又引出一个问题，如果消费的时候失败了呢？ 那么它重复消费的时候先去判断了，判断已经有幂等标识了，同样也不去消费了，所以有个改进：就是把消费逻辑try起来，如果有异常也就是没有消费成功，抛出异常同时把幂等的redis标识给删掉，这样方便后面mq重试的时候能继续消费。同时使用Throwable 去捕获，  
这样都能捕获的到。
😧😧😧😧😧😧😧😧😧😧😧😧😧😧😧😧😧！！！！！亮点                        同样又引出了一个问题，如果catch快里面的内容没有进行呢？比方说kill-9直接强制结束线程也就是try里面都没走完，网络断开导致线程卡死，Redis客户端永久阻塞卡住等。
这样就体现了我们设置幂等标识为2分钟的好处了，这时候判断完幂等标识后，也就是幂等标识是存在的-同时判断是否消息是否执行完毕（在极端情况下try块没走完也抛异常但是catch快没执行，我们不知道消息是否被消费完。）这时候我们可以设置幂等标识key的时候同时设置一个value，设计1为完成，0为执行中，也就是未完成。  
这样我们就把设置value为1也就是消费完成这个放在最后面 try catch 块的后面。同时判断幂等标识里面如果判断执行未完成throw异常，mq也会一直重试，可能1个小时后，或者几个  小时后重试，这时候10分钟肯定是过去了，所以消息也能够正常的消费了。消费完毕后也会设置完成标识为1了。如果设置有点大了，比方说5分钟，5分钟如果有100万  
的幂等key，会比较浪费我们的内存。
问题： 如何应对海量幂等key所消耗的内存。
         1：MySQL或者其他大数据量存储。
         2：想办法改造数据比方说 布隆过滤器或者happyloglog 但又引入新的问题，布隆过滤器误判和容量。hyperLogLog只用12KB就能支持几亿级UV统计，误差约1%，在监控统计  
         场景可以接收，同时Redis提供了原生的PFADD/PFCOUNT，性能和扩展性都非常好，PFADD key element1 element2 ...  
                                                                                 PFCOUNT key  
                                                                                 PFMERGE destkey key1 key2 ...  

# Redis命名规范
设置一个常量类记录shortlink：login short-link:lock类似这样的key前缀.

# 添加初始化任务
初始化短链接监控消息队列消费者组 Redis  stream topic和消费者组    
