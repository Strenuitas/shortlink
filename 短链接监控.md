...
# 短链接不支持换域名  
# 获取用户的操作系统
 本质上是从request的头里面拿到它对应的User-Agent

# 获取浏览器类型
同上 调同样的api获取

# 统计高频IP访问
做起来会比较复杂，我们可以以mysql来做，所以要记录一个访问监控日志，统计里面IP的count然后排行，逆序排列就好。日志记录一系列信息同时附带user信息，
# 统计短链接访客类型
在监控日志的基础上，选择一个时间段，如果是首次访问者就是新访客，若是在这段时间之前就已经访问过，那么就是老访客。可以选中当天的日期查看老访客的数量，如果老访客的数量较多，就说明您也就是该用户下投放的短链接很受欢迎，用户会持续来访问。具体业务逻辑就是选择一段时间    
的监控日志中的user信息，然后组一个group去之前的监控日志去查有没有该用户  

# 短链接设备访问记录
同样的因为使用了MySQL的特性 INSERT ... ON DUPLICATE KEY UPDATE （简称upsert）。也就是有就更新，没有就插入，这种不是Mybati-plus内置的简单的CRUD可以搞定的功能，因此必须自定义sql语句 也就是写在linkDeviceStatsMapper下面要写对应的insert方法，同样记录在监控日志里面

# 短链接访问网络记录
要先获取到ip通过ip确定是wifi还是移动网络

#  单个短连接的监控
前端传过来单个短连接的 短链接，分组gid，时间参数（开始日期和结束日期），然后通过已有的监控表把数据都拿出来作为单个短链接的监控，先去判断有没有访问记录也就是linkAccessStatsMapper里去查有没有该短链接的访问记录，如果没有返回空，前端去做空的渲染，也就是全部设置为0  
然后做基础访问：对应日期的pv uv uip；  具体业务逻辑：存到数据库里面的查的是linkAccessStats这张表，先根据查询的起止时间设置好一个时间范围的参数List<String> rangeDates，比方说查两天 也就是有2个成员，然后foreach去比对该表中每天对应的记录，如果有值就拿出来，没值设置为0
然后是地区记录：国内的地区记录，然后是24小时记录，高频访问ip，一周访问，访问浏览器记录，操作系统访问
访客类型：新老访客，最后把这些数据全部构造成一个ShortLinkStatsRespDTO的相应参数，返回给前端
#  分页查询单个短连接的监控日志
是要去操作我们的短链接访问监控这个表的，查到对应单个短链接的然后分页查询。分页查询都要继承page前端传过来是第几页，每页多少，😧😧😧😧😧😧😧😧😧😧😧😧😧😧😧新老访客这里比较复杂，可以详细拿来讲给面试官.

# 短链接访问监控
linkAccessLogs,同样是处在跳转的时候加入一条信息到监控日志里面

# 短链接分页查询 今日 累计 访问的基础信息 pv uv uip
累计访问可以加到link表里面，关键是今日访问次数 可以单独设置一个t_link_today_stats来记录 包含 gid 短链接 today_pv today_uv today_uip  ，这样我们去分页查询短链接的时候join这个表 设置今日的日期就可以获取到今日的基础访问记录，同时这里要有一个绑定表的设计，不然会造成笛卡尔积的查询结果。

# 单个短连接指定时间内监控数据
把sql语句实现就好，在linkAccessLog里面去查指定时间的数据。

# 分组短链接监控数据    查的比较多，一般生产的限流比着单个短连接监控数据要大
类似单个，只不过把fullshorturl字段给删掉就好。
