...
```
然后业务逻辑就是 注册好一个用户后，自动给设置一个默认分组便于存储短链接


# 修改短链接
先把创建类型设置一个枚举，ValidDateTypeEnum方便我们后面好判断。枚举类的成员都算做它的实例对象。  
修改可以修改 原始短链接  分组标识 有效期  描述 
因为我们创建短链接的时候，可能会设置永久有效，所以先进行一个判断。看是不是永久有效，它的有效期就是空的
修改的逻辑 是先找到要变更的短链接，然后再设置新的ShortLinkDO去进行update,这里会有缓存穿透的问题
而且因为gid可以变更，如果变更前后gid一样，就可以在同一个group里面进行修改，如果gid变更，也就是换组了，则需要先把原来的组给删掉，然后再找的新的gid所属的分组在进行插入。
涉及到数据库的先删后新增，所以要有一个rollback 要加注解    @Transactional(rollbackFor = Exception.class)    事务回滚，修改短链接方法里面要么同表修改，要么不同表 先删除再插入，goto表也要同步修改，三者要么一起完成，要么全都不完成。
同表修改，正常lambdaUpdate使用shortLinkDO + updateWrapper更加安全。 不同表修改，按上方写。

# 短链接跳转 😧😧😧😧😧😧😧 核心😧😧😧😧😧😧
访问浏览器输入短网址，然后到nginx，代理到中台服务，获取短网址对应的原始网址，302重定向到原网址。
