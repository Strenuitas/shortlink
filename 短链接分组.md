...
完整数据流程：
用户登录-存储信息到Redis的hash数据结构中具体admin业务里面提到，然后返回给前端一个token，然后前端之后的请求都会带着这个一个username和token来访问网关，网关根据username和token验证是否是当前在登陆的用户，如果是，则将userid和realname放入新的header，先传入admin服务的UserTransmisFilter读取新的header，然后构建UserInfoDO对象放入到userContext对象中，使用ThreadLocal存储，以供之后转发到其他的服务使用。这里用户信息过滤器使用了阿里的TTL，能在线程池，异步任务中传递，项目多处异步时避免用户信息丢失。多线程下比较安全。且请求结束后，清理ThreadLocal，UserContext.removeUser(),不然会一直占用tomcat线程池的线程。
  
# 修改短链接:
通过分组标识gid修改短链接name

# 短链接分组排序
前端传过来排序好的分组标识加sortOrder，然后存储到数据库里面设置成新的排序。
